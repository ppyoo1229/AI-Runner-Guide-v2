...
serve(async (req) => {
  ...
  try {
    ...
    console.log(`[parse_running_query_enhanced] Processing query: ${query}`)
    ...
    if (hfTgiUrl) {
      ...
      try {
        ...
        console.log('[parse_running_query_enhanced] LLM parsing successful')
      } catch (error) {
        console.log(`[parse_running_query_enhanced] LLM parsing failed, using fallback: ${error.message}`)
        parsedParams = parseWithFallback(query)
      }
    } else {
      // Fallback 파싱 사용
      console.log('[parse_running_query_enhanced] Using fallback parsing (no LLM server)')
      parsedParams = parseWithFallback(query)
    }
    ...
    console.error('[parse_running_query_enhanced] Error:', error)
    return new Response(
...
function parseWithFallback(query: string): ParsedParams {
  ...
  const locationPatterns = [
    /(?:에서|근처|주변)\s*(.+?)(?:\s|$)/,
    /(.+?)(?:에서|근처|주변)/,
    /(잠실|강남|여의도|한강|올림픽공원|남산|청계천|서울숲|해운대|광안리|송도|수성못|갑천|태화강|제주)/i
  ]
  ...
  // 특별 키워드 추가
  if (query.match(/해변|바다|해안/i)) {
    params.keywords = [...(params.keywords || []), '해변', '바다', '해안']
  }
  if (query.match(/공원|둘레길/i)) {
    params.keywords = [...(params.keywords || []), '공원', '둘레길', '자연']
  }
  if (query.match(/마라톤|대회|훈련/i)) {
    params.keywords = [...(params.keywords || []), '마라톤', '대회준비', '훈련']
  }
